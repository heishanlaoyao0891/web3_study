...
func main() {
	...
	sqls := []string{
		...
		`ALTER TABLE users ADD COLUMN level INT DEFAULT 1 COMMENT '等级'`,
		`ALTER TABLE users ADD COLUMN exp INT DEFAULT 0 COMMENT '经验值'`,
		`ALTER TABLE users ADD COLUMN coins INT DEFAULT 0 COMMENT '金币'`,
		...
		`INSERT IGNORE INTO tags (name, color, sort_order) VALUES
		('Java', '#f89820', 1),
		('Go', '#00add8', 2),
		('Python', '#3776ab', 3),
		('前端', '#42b883', 4),
